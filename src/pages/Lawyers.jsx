...
import AdvocateProfileSidebar from '../components/AdvocateProfileSidebar'

const Lawyers = () => {
  const [selectedAdvocate, setSelectedAdvocate] = useState(null)
  const [isSidebarOpen, setIsSidebarOpen] = useState(false)

  const handleAdvocateClick = (advocate) => {
        ...
        <div className="w-full bg-gray-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="flex items-center justify-between mb-8">
              <h1 className="text-3xl font-bold text-gray-900">Registered Advocates</h1>
              <div className="flex gap-4">
                <Link
                  to="/registration"
                  className="px-4 py-2 bg-[#DC1F27] text-white rounded hover:bg-[#b01820] transition-colors text-sm"
                >
                  Register
                </Link>
                <Link
            ...
            <AdvocatesList onAdvocateClick={handleAdvocateClick} />
          </div>
        </div>
      </div>

      {/* Profile Sidebar */}
      <AdvocateProfileSidebar
